...
{
    class AmbientObscurance : ImageEffectBase
    {
        //@Range (0,3)	
        ...
        {
            //CheckSupport(true);
            aoMaterial = CheckShaderAndCreateMaterial(shader, aoMaterial);
            if (!aoMaterial)
            {
                return false;
            }
        ...
        {
            Debug.Log("Destroying AO material");
            if (aoMaterial)
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            {
                Debug.Log("AmbientObscurance shader or material not ready");
